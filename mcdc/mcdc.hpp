...
protected:
	...
	std::pair<McdcType, cchar> getMcdcType(AST& ast);
	...
	void checkForCoverage(void);

	// Helps to select "good" test pairs
	std::pair<bool, McdcIndependencePair> isCompleteTestPairInCoverageResult(McdcIndependencePairSet& mips, CellVectorHeaderSet& cvhs);
	std::pair<bool, McdcIndependencePair> isOnePartIfTestPairInCoverageResult(McdcIndependencePairSet& mips, CellVectorHeaderSet& cvhs);


	// Best cost and heuristic function for selecting minimum test pairs
	sint compare2TestValues(const TableCellVector &left, const TableCellVector &right);
	sint calculateSumForOneTest(uint test);	// Favours unique cause MCDC over others
